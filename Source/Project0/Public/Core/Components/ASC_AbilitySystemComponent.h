...
#include "ASC_AbilitySystemComponent.generated.h"


DECLARE_DYNAMIC_MULTICAST_DELEGATE_ThreeParams(FReceivedDamageDelegate, UASC_AbilitySystemComponent*, SourceASC, float,
                                               UnmitigatedDamage, float, MitigatedDamage);

/** Delegate for when an effect is applied */
DECLARE_MULTICAST_DELEGATE_TwoParams(FOnGameplayEffectDurationChanged, UASC_AbilitySystemComponent*, const FActiveGameplayEffect&);
...
*/
USTRUCT()
struct PROJECT0_API FGameplayAbilityLocalAnimMontageForMesh
{
	GENERATED_BODY();
...
*/
USTRUCT()
struct PROJECT0_API FGameplayAbilityRepAnimMontageForMesh
{
	GENERATED_BODY();
 ...
 */
UCLASS()
class PROJECT0_API UASC_AbilitySystemComponent : public UAbilitySystemComponent
{
	GENERATED_BODY()

public:
	UASC_AbilitySystemComponent();


	virtual void InitAbilityActorInfo(AActor* InOwnerActor, AActor* InAvatarActor) override;


	typedef TFunctionRef<bool(const UGA_MainGameplayAbility* Ability, FGameplayAbilitySpecHandle Handle)> TShouldCancelAbilityFunc;
	...
	uint8 CharacterAbilitiesGiven:1;
	UPROPERTY(BlueprintReadOnly)
	uint8 StartupEffectsApplied:1;
	...
	// Called from BDamageExecCalculation. Broadcasts on ReceivedDamage whenever this ASC receives damage.
	virtual void ReceiveDamage(UASC_AbilitySystemComponent* SourceASC, float UnmitigatedDamage, float MitigatedDamage);
	UFUNCTION(NetMulticast, Reliable)
	virtual void Multicast_ReceiveDamage(UASC_AbilitySystemComponent* SourceASC, float UnmitigatedDamage, float MitigatedDamage);
	...
	FOnGameplayEffectDurationChanged OnGameplayEffectDurationChanged;
	...
	void AddAbilityToActivationGroup(EP0AbilityActivationGroup Group, UGA_MainGameplayAbility* Ability);
	void RemoveAbilityFromActivationGroup(EP0AbilityActivationGroup Group, UGA_MainGameplayAbility* Ability);
	void CancelActivationGroupAbilities(EP0AbilityActivationGroup Group, UGA_MainGameplayAbility* IgnoreAbility, bool bReplicateCancelAbility);
...
// Set if montage rep happens while we don't have the animinstance associated with us yet
	...
	void OnPredictiveMontageRejectedForMesh(USkeletalMeshComponent* InMesh, UAnimMontage* PredictiveMontage) const;
...
protected:
	...
	int32 ActivationGroupCounts[(uint8)EP0AbilityActivationGroup::MAX];
